...
services="$ELK_ES_IMAGE_TAG $ELK_KIBANA_IMAGE_TAG $ELK_LOGSTASH_IMAGE_TAG $ELK_DEJAVU_IMAGE_TAG"
for service in $services
do
    if [ -n "$( docker images --filter reference="$service" -q )" ]; then
        log info "Delete the \"$service\" tag"
        docker rmi $service 1>/dev/null
    fi
done
