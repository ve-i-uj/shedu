...
log info "*** Game status (the current game is \"$GAME_NAME\") ***"
log info

log info "* The built images:"
log info "-----"
# Сперва распечатать для текущего проекта, потом всех остальных
game_res=$( docker images --filter reference="$PROJECT_NAME/*" --format "{{.Repository}}:{{.Tag}}" | grep "$GAME_NAME")
other_res=$( docker images --filter reference="$PROJECT_NAME/*" --format "{{.Repository}}:{{.Tag}}" | grep -v "$GAME_NAME")
